...
class BasketAnalysis:
    def __init__(self,):
        self.table = pd.read_csv(
            "tswebserver/utils/database/basket_analysis.csv",)
...
class ForecastTask:
    def __init__(self,):
        self.table = pd.read_csv(
            "tswebserver/utils/database/tsbyproduct.csv", index_col="Periodo")
        self.articulos = pd.read_csv(
            "tswebserver/utils/database/articulos.csv")
        self.articulos.drop_duplicates(subset=['idArticulo'], inplace=True)

    def forecast(self, id_articulo, ahead, before=12):
        mod = sm.tsa.statespace.SARIMAX(self.table[id_articulo], order=(
            1, 1, 0), seasonal_order=(2, 1, 0, 12), freq='MS')
        mod_trained = mod.fit(disp=False)
        before = self.table[id_articulo].tail(before)
        name = self.articulos.query(f"idArticulo=={id_articulo}")[
            "DescProducto"].tolist()[0]
    ...
    def format_output(before, model, n_ahead):
        ...
        for _ in range(n_ahead):
            finalmonth = datetime.strptime(
                str(before_index[-1]), '%Y-%m-%d').date()
